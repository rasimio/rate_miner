...
from app.model import TransactionModel
...
class TransactionCRUD:
    ...
    async def get(self, tx_id: str) -> Optional[TransactionModel]:
        query = """
            select
        ...
        if effect is not None:
            return TransactionModel(**effect)

        return effect

    async def get_validator(self, tx_id: str) -> Optional[bool]:
        query = """
    ...
    async def fetch(
        self, reflector: bool, reflector_publisher: bool
        ...
        query = """
            ...
            inner join token on transaction.token_id = token.id
            where reflector = $1
            and reflector_publisher = $2;
        """
        effect = await self.__cursor.fetch(query, reflector, reflector_publisher)

        return [TransactionModel(**e) for e in effect]
